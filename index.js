...
$('document').ready(function () {
    ...
    $('#search').on('input', function (e) {
        ...
        for (var i = 0; i < rows.length; i++) {
            var name = $(rows[i]).find('td').eq(0).text();
            var age = $(rows[i]).find('td').eq(1).text();
            var sem = $(rows[i]).find('td').eq(2).text();
            var reg = $(rows[i]).find('td').eq(3).text();
            if (name.toLowerCase().includes(search.toLowerCase()) || age.includes(search) || sem.includes(search) || reg.includes(search)) {
                $(rows[i]).show();
            }
            else {
                $(rows[i]).hide();
                ...
                if (check == value) {
                    ...
                    for (var j = 0; j < 4; j++) {
                        ...
                        if (value == 1) {
                          ...
                          else { cell.attr('contenteditable', 'false');
